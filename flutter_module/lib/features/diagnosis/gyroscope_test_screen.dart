...
class GyroscopeTestScreen extends ConsumerStatefulWidget {
  const GyroscopeTestScreen({super.key});

  @override
  ConsumerState<GyroscopeTestScreen> createState() => _GyroscopeTestScreenState();
}

class _GyroscopeTestScreenState extends ConsumerState<GyroscopeTestScreen> {
  final double _screenProgress = ProgressCalculator.getProgressForScreen('gyrosensor'); // Dynamic progress for screen 26
  GyroscopeTestState _currentState = GyroscopeTestState.initial;
  ...
  void _scheduleAutoStartIfNeeded() {
    ...
    if (!ref.read(testImagesProvider.notifier).getIsAutoMode(TestConfig.testIdGyrosensor)) return;
    if (_currentState != GyroscopeTestState.initial || !_hasSensor) return;
    _autoStartScheduled = true;
    Future.delayed(testAutoModeStartDelay, () {
      if (mounted && _currentState == GyroscopeTestState.initial && _hasSensor) {
        _startTest();
      }
    });
  }
  ...
  bool _shouldShowStartTestButton() {
    return !ref.read(testImagesProvider.notifier).getIsAutoMode(TestConfig.testIdGyrosensor);
  }
  ...
  Future<void> _checkGyroscopeAvailability() async {
    try {
      // Try to listen to gyroscope for a short time to check if sensor exists
      ...
      setState(() {
        ...
        if (!isAvailable) {
          _currentState = GyroscopeTestState.sensorNotAvailable;
        }
      });
      _scheduleAutoStartIfNeeded();
    } catch (e) {
      print('❌ Error checking gyroscope sensor: $e');
      setState(() {
        _isChecking = false;
        _hasSensor = false;
        _currentState = GyroscopeTestState.sensorNotAvailable;
  ...
  void _startTest() {
    ...
    setState(() {
      _currentState = GyroscopeTestState.testing;
      _xValues.clear();
      _yValues.clear();
      _zValues.clear();
    ...
    _testTimer = Timer(const Duration(seconds: 5), () {
      _gyroscopeSubscription?.cancel();
      setState(() {
        _currentState = GyroscopeTestState.resultReady;
      });
    });
  }
  ...
  Future<void> _navigateToNextTest() async {
    ...
    if (testParameters != null) {
      TestNavigationService.navigateToNextTest(
        context,
        testParameters,
        TestConfig.testIdGyrosensor,
      );
    } else {
      debugPrint('❌ GyroscopeTestScreen: Failed to get test parameters for navigation.');
      TestNavigationService.navigateToSummaryScreen(context);
    }
  }

  void _handleTestPass() {
    TestResultHelper.savePass(ref, TestConfig.testIdGyrosensor);
    _navigateToNextTest();
  }

  void _handleTestFail() {
    TestResultHelper.saveFail(ref, TestConfig.testIdGyrosensor);
    _navigateToNextTest();
  }

  void _handleSkip() {
    TestResultHelper.saveSkip(ref, TestConfig.testIdGyrosensor);
    _navigateToNextTest();
  }
          ...
          Column(
            children: [
                  ...
                  child: SingleChildScrollView(
                    child: Column(
                      ...
                      children: [
                        if (_isChecking)
                        ...
                        else if (_currentState == GyroscopeTestState.initial)
                          // Initial state - show instruction
                          CommonContentWidget(
                            customImageWidget: CommonTestImage(
                              screenName: TestConfig.testIdGyrosensor,
                              isPassed: true,
                              localFallbackPath: AppStrings.imageGyroscopePath,
                              fallbackIcon: Icons.my_location,
                              width: 120,
                              height: 120,
                            ),
                            heading: AppStrings.testingGyroscopeSensor,
                            subheading: AppStrings.gyroscopeTestInstruction,
                          )
                        else if (_currentState == GyroscopeTestState.sensorNotAvailable)
                          ...
                          ScanResultCard(
                            ...
                            title: AppStrings.gyroscopeNotDetected,
                            subheadingLines: [
                              AppStrings.gyroscopeNotDetectedMessage,
                            ],
                            showStatusIcon: false,
                          )
                        else if (_currentState == GyroscopeTestState.testing)
                            ...
                            children: [
                              ...
                              Container(
                                ...
                                child: Column(
                                  crossAxisAlignment: CrossAxisAlignment.start,
                                  children: [
                                    // Gyroscope value (as shown in image)
                                    Text(
                                      'Gyroscope value: ${((_x.abs() + _y.abs() + _z.abs()) / 3).toStringAsFixed(2)}',
                        ...
                        else if (_currentState == GyroscopeTestState.resultReady)
                          ...
                          ScanResultCard(
                            ...
                            title: _sensorWorking 
                                ? AppStrings.gyroscopeWorking 
                                : AppStrings.gyroscopeNotWorking,
                            subheadingLines: _sensorWorking 
                                ? [AppStrings.gyroscopeWorkingMessage]
                                : [AppStrings.gyroscopeNotWorkingMessage],
                            showStatusIcon: true,
                          ),
                      ],
                    ),
              ...
              Padding(
                ...
                child: Column(
                  children: [
                    if (_currentState == GyroscopeTestState.initial && _shouldShowStartTestButton())
                      CommonButton(
